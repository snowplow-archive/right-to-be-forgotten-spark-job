...
class RightToBeForgottenJobSpec extends fixture.FunSpec {
  ...
  describe("Right to be forgotten job") {
    it("Must separate the events that contain the identifiers from those that do not") { tempDir: Path =>
      ...
      val arguments = Array[String](
        "--removal-criteria",
        RightToBeForgottenJobSpec.R2fRemovalCriteria,
        "--input-directory",
        RightToBeForgottenJobSpec.InputDirectory,
      ...
      val originalInput = Source
        .fromFile(RightToBeForgottenJobSpec.InputDirectory + "part-00057-09103e46-8f45-49ba-a1bc-26a869e69633-c000.csv")
        .getLines
        .toList
    ...
    it("Must fail execution when more than the maximum proportion of events match") { tempDir: Path =>
      val outputMatchingDataDir    = getMatchingOutputDir(tempDir)
      val outputNonMatchingDataDir = getNonMathcingOutputDir(tempDir)
      ...
      assertThrows[MaximumRightToBeForgottenMatchingRecordsViolation](r2fJob.run(spark))
      spark.stop()
    }
  ...
  private def runJobWithArguments(tempDir: Path,
...
object RightToBeForgottenJobSpec {
  val InputDirectory                 = "src/test/resources/enriched/archive/"
