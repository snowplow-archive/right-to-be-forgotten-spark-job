...
class R2FConfArguments(arguments: Seq[String]) extends ScallopConf(arguments) {
  val removalCriteria = opt[String](name = "removal-criteria", required = true)
...
case class ValidatedR2FJobConfiguration(arguments: R2FConfArguments) {
  ...
  val removalCriteriaFile            = arguments.removalCriteria()
  ...
  val removalCriteriaLines           = ValueFilters.getRemovalCriteriaLines(removalCriteriaFile)
  val valueFilters = ValueFilters
    .createFiltersFromRemovalCriteria(removalCriteriaLines)
    .fold(errorList => throw new InvalidDataInRightToBeForgottenConfiguration(errorList.mkString("\n")), vF => vF)
}
